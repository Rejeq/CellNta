...
struct EigeneHasher
{
	size_t operator()(T const& data) const
	{
		...
		for (size_t i = 0; i < data.size(); ++i)
			seed ^= std::hash<typename T::Scalar>()(data(i)) + 0x9e3779b9 + (seed << 6) + (seed >> 2);
		return seed;
	}
};
...
struct Area
{
  ...
  static std::vector<Area> InverseClip(const Area& clipper, const Area& subject);
...
public:
  ...
  void Set(int x, int y, int z) { m_pos.x() = x; m_pos.y() = y; m_pos.z() = z;}
  ...
  bool operator==(const ChunkPos& other) const { return (m_pos == other.m_pos); }
  bool operator!=(const ChunkPos& other) const { return !(*this == other); }
  ChunkPos operator+(const ChunkPos& r)  const { return ChunkPos(m_pos + r.m_pos); }
...
{
public:
	using Map = std::unordered_map<Eigen::VectorXi, state_t, EigeneHasher<Eigen::VectorXi>>;
...
public:
  ...
  RenderData();
	...
	void SetDimensions(size_t dim) { m_data.clear();  m_d = dim; }
  ...
  void SetCollatingX(int x) { m_collX = x; m_desiredArea.push_back(GetDistanceArea()); }
  void SetCollatingY(int y) { m_collY = y; m_desiredArea.push_back(GetDistanceArea()); }
  void SetCollatingZ(int z) { m_collZ = z; m_desiredArea.push_back(GetDistanceArea()); }
