...
class HypercubeStorage {
 public:
  ...
  size_t GetPointsSizeInBytes() const {
 ...
 private:
  ...
  inline size_t GetRamaindingFaces(const size_t& dim) const {
    assert(dim > 1);
    return ((size_t)1) << (dim - 2);
  }
  inline size_t GetFacesCount(const size_t& dim) const {
    return GetPlanesCount(dim) * GetRamaindingFaces(dim);
  }

  inline size_t GetVerticesCount(size_t dim) const {
    return ((size_t)1) << dim;
  }
