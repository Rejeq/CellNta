...
void HypercubeStorage::GenerateIndicesPolygon()
{
  ...
  for(size_t offset = 0; offset < maxVert; offset = (offset == 0) ? 1 : offset << 1)
  {
    for(size_t corner = 1; corner < maxVert; corner <<= 1)
    {
      ...
      for(size_t edge = corner << 1; edge < maxVert; edge <<= 1)
      {
        ...
        data[0] = offset;          data[3] = offset;
        data[1] = corner + offset; data[4] = edge + offset;
        data[2] = total;           data[5] = total;
        ...
        if(!usedTotal[total])
        {
          ...
          {
            if(conCorner == corner || conCorner == edge ||
                conCorner == offset)
              continue;

            for(size_t conEdge = conCorner << 1; conEdge < maxVert; conEdge <<= 1)
            {
              ...
              data[0] = total;             data[3] = total;
              data[1] = conCorner + total; data[4] = conEdge + total;
              data[2] = conTotal;          data[5] = conTotal;
